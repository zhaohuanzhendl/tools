...
{  
    ...
    {  
        cvector_iter_val(cv, iter, &tt);  
        printf("var:%s at:%d\n", (&tt)->str, cvector_iter_at(cv, iter));  
    }  
...
int main()
{
    ...
    */
    test_t tt;
    tt.str = (char *)malloc(6);
    strncpy(tt.str, "Hello", 5);
    tt.str[5] = '\0';
    tt.a = 10;
    cvector cv = cvector_create(sizeof(test_t));

    cvector_pushback(cv, &tt);
    cvector_pushback(cv, &tt);
